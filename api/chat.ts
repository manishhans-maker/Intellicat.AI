  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    for (const model of GEMINI_CANDIDATE_MODELS) {
      try {
        ...
        console.warn(`Gemini model ${model} attempt ${attempt + 1} failed in Vercel function:`, err?.message);
        lastErr = err;
        if (attempt === 0) {
    ...
    if (activeProvider === "groq") {
      ...
      try {
        ...
        try {
          ...
          console.warn(`Groq error with model ${selectedModel}, falling back to llama-3.1-8b-instant:`, modelErr?.message);
        ...
        console.error("Groq execution failed in Vercel function:", groqFailure);
